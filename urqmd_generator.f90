...
contains
  ...
  function generator_run(self, err) result(ed)
    ...
    call ini%set_pdg_code_a(pdga)
    call ini%set_pdg_code_b(pdgb)
    call ini%set_p_za(pza)
    call ini%set_p_zb(pzb)
    call ini%set_energy(eout)
    call ini%set_sect_nn(real(snnout, kind(0.0)))
    call ini%set_b(real(bimp_val, kind(0.0)))
    call ini%set_num_coll(nco)
    call ini%set_num_coll_pp(npp)
    call ini%set_num_coll_pn(npn)
    call ini%set_num_coll_nn(nnn)
    call ini%set_num_part(npt)
    call ini%set_num_part_a(npa)
    call ini%set_num_part_b(nbt)
    call ini%set_phi_rot_a(real(phia, kind(0.0)))
    call ini%set_theta_rot_a(real(thaa, kind(0.0)))
    call ini%set_phi_rot_b(real(phib, kind(0.0)))
    call ini%set_theta_rot_b(real(thab, kind(0.0)))
    call ini%set_ini_state_particles(ini_parts)
    if (np < 0) np = 0
    call ed%set_ini_state(ini)
