...
def train_ppo(
    ...
    *,
    ...
    ``lr_scheduler.step()`` runs **once per outer update** after PPO optimization (and optional
    anchor BCE step). If ``anchor_bce_coef > 0``, an extra ``optimizer.step()`` runs after the
    PPO minibatches (BCE on ``ac.policy`` only), mirroring :func:`training.reinforce.train_reinforce`.
    """
    opt = optimizer
    ...
    try:
        for u in pbar:
            ...
            if anchor_bce_coef > 0.0 and buffer:
                opt.zero_grad()
                for ep in buffer:
                    obs_anchor = env.reset(
                        ep["event_pos"], ep["event_mom"], ep["event_isp"]
                    )
