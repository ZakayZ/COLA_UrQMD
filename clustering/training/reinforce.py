...
def train_reinforce(
    ...
    *,
    ...
    ``anchor_bce_coef``: if positive, add a supervised-style BCE anchor toward spatial–momentum
    baseline edge targets each update (stabilizes logits vs pure RL drift).
    ...
    try:
        for u in pbar:
            ...
            if anchor_bce_coef > 0.0 and batch_obs:
                for i in range(len(batch_obs)):
                    # Fresh ``obs`` must match ``env`` after reset (``batch_obs[i]`` is stale).
                    obs_anchor = env.reset(batch_pos[i], batch_mom[i], batch_isp[i])
                    logits = policy(obs_anchor)
